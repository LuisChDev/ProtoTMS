...
import itertools
import gmaps  # type: ignore
import googlemaps  # type: ignore
from yaml import safe_load
from typing import Dict, List

with open("secrets.yaml") as fl:
    keys: Dict = safe_load(fl)
...
def _distance_calculator(_df):
    ...
    for i in range(len(_df)):
        _df["latitude-longitude"].iloc[i] = (
            str(_df.latitude[i]) + "," + str(_df.longitude[i])
        )
    ...
    for i in range(len(_df)):
        for j in range(len(_df)):
            ...
            _google_maps_api_result = googlemaps.directions(
                _df["latitude-longitude"].iloc[i],
                _df["latitude-longitude"].iloc[j],
                mode="driving",
            )
            # append distance to result list
            _distance_result[i][j] = _google_maps_api_result[
                0]["legs"][
                    0]["distance"][
                        "value"]
...
for vehicle_count in range(1, vehicle_count + 1):
    ...
    # foluma (3)
    for k in range(vehicle_count):
        problem += pulp.lpSum(x[0][j][k] for j in range(1,
                                                        customer_count)) == 1
        problem += pulp.lpSum(x[i][0][k] for i in range(1,
                                                        customer_count)) == 1
    ...
    for k in range(vehicle_count):
        for j in range(customer_count):
            problem += (
                pulp.lpSum(x[i][j][k] if i != j else 0 for i in range(
                    customer_count))
    ...
    for k in range(vehicle_count):
        problem += (
            pulp.lpSum(
                df.demand[j] * x[i][j][k] if i != j else 0
                for i in range(customer_count)
                for j in range(1, customer_count)
            )
            <= vehicle_capacity
...
for i in range(customer_count):
    if i == 0:
        ...
        plt.text(df.latitude[i], df.longitude[i],
                 str(df.demand[i]), fontsize=12)
...
for k in range(vehicle_count):
    for i in range(customer_count):
        for j in range(customer_count):
            if i != j and pulp.value(x[i][j][k]) == 1:
                plt.plot(
                    [df.latitude[i], df.latitude[j]],
                    [df.longitude[i], df.longitude[j]],
                    c="black",
                )
